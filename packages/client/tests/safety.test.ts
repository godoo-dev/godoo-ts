...
import type { OperationInfo, SafetyContext } from '../src/index.js';
import {
  configFromEnv,
  DELETE_METHODS,
...
describe('Error Hierarchy', () => {
  ...
  describe('OdooNetworkError', () => {
    ...
    it('has toJSON with NETWORK_ERROR code', () => {
      ...
      expect((json.details as unknown).cause).toBe('cause');
    });
  });
  ...
  describe('OdooSafetyError', () => {
    ...
    it('has toJSON with SAFETY_BLOCKED code', () => {
      ...
      expect((json.details as unknown).operation.name).toBe('odoo.unlink');
    });
  });
