...
export class CodeGenerator {
  ...
  async generate(options: CodeGeneratorOptions = {}): Promise<string> {
    ...
    for (const model of models) {
      try {
        const metadata = await this.introspector.getModelMetadata(model.model, { bypassCache });
        allMetadata.push(metadata);
      } catch (error) {
        console.warn(
          `[odoo-introspection:codegen] Warning: Failed to introspect ${model.model}`,
          error,
        );
      }
    }
    ...
    console.log(
      `[odoo-introspection:codegen] Successfully introspected ${allMetadata.length} models`,
    );
    ...
    if (outputDir) {
      ...
      console.log(
        `[odoo-introspection:codegen] Generated file: ${path.join(outputDir, 'generated.ts')}`,
      );
    }
